...
@if (strpos($file->mime, 'image') !== false)
    <a href="{{ url('media/oprecord/download/' . $file->filename) }}"><img class="img-thumbnail img-responsive" src="{{ asset('previews/' . $file->filename) }}" alt="{{ $file->orginal_filename }}"></a>
@elseif (strpos($file->mime, 'video') !== false)
    ...
    <a href="{{ url('media/oprecord/download/' . $file->filename) }}" class="btn btn-info btn-block m-t-sm">Descargar <i class="fa fa-download"></i></a>
    <script>
