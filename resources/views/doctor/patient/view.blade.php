...
@section('content')
                    ...
                    <div class="wrapper">
                        ...
                        <div>
                            ...
                            <ul class="list-unstyled inf-list">
                                <li>
                                    <i class="fa fa-building user-profile-icon"></i> {{ $patient->medical_insurance == 'Y' ? 'Sí' : 'No' }}
                                </li>
                                <li>
                                    <i class="fa fa-building user-profile-icon"></i> {{ $patient->medical_insurance == 'Y' ? $patient->medical_insurance_name : 'N/A' }}
                                </li>
                                <li>
                        ...
                        <div id="patient-full-info" class="tab-pane active">
                            <div class="padder">
                                @if ($patient->doctor['id'] == 3)
                                    ...
                                    <div class="line"></div>
                                    <h6><strong>Nombre Seguro médico: </strong>{{ $patient->medical_insurance == 'Y' ? $patient->medical_insurance_name : 'N/A' }}</h6>
                                    <div class="line"></div>
    ...
    @if ($patient->doctor['id'] == 2)
    @include('doctor.patient.update')
    @elseif ($patient->doctor['id'] == 3)
...
<script src="{{ asset('assets/js/autosize/autosize.min.js') }}"></script>
<script>
    $(function () {
        ...
        $('#view-edit').on('click', function(e) {
            ...
            @if ($patient->doctor['id'] == 2)
            var url = "{!! url('patients/info') !!}";
            @elseif ($patient->doctor['id'] == 3)
            var url = "{!! url('pma-patients/info') !!}";
            @endif;
        ...
        $(document).on('click', '.dt-update-history', function (e) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    autosize($('#update-history-form textarea[name=description]'));
                    loader.fadeOut();
                }
            })
        });
        ...
        $(document).on('click', '.dt-update-evolution', function (e) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    autosize($('#update-patient-evolution-form textarea[name=description]'));
                    loader.fadeOut();
                }
            })
        });
