...
use App\DRDSB\File\SurgeryFileEntry;
...
class SurgeriesFilesController extends Controller
{
    ...
    public function getFilesData(Request $request)
    {
        ...
        $datatables = Datatables::of(SurgeryFileEntry::where('user_id', $this->auth->id())->where('patient_id', $request->get('pid')));
        ...
        $datatables->addColumn('thumbnail', function ($fileEntry) {
            ...
            if (strpos($fileEntry->mime, 'image') !== false) {
                ...
                return '<a href="#" class="dt-view-surgery-file" data-remodal-target="file-viewer" data-fid="' . $fileEntry->id . '"><img class="img-thumbnail-table" src="' . $src . '"></a>';
                ...
                return '<a href="' . url('media/surgery/download/' . $fileEntry->filename) . '"><img class="img-thumbnail-table" src="' . $src . '"></a>';
            }
            ...
            return
                '<div class="btn-group">
                    ...
                    <ul class="dropdown-menu pull-right">
                        <li><a data-fid="' . $fileEntry->id . '" data-remodal-target="update-surgery-file" class="dt-update-surgery-file" href="#">Editar información archivo</a></li>
                        <li><a data-fid="' . $fileEntry->id . '" data-remodal-target="remove-surgery-file" class="dt-remove-surgery-file" href="#">Eliminar archivo</a></li>
                    </ul>
                </div>';
        });
    ...
    {
        $fid  = $request->get('fid');
        $file = SurgeryFileEntry::find($fid);

        return view('doctor.media.patient.surgery.view', ['file' => $file]);
    }
    ...
    {
        $file = SurgeryFileEntry::find($id);

        return view('doctor.media.patient.surgery.ajax-update', ['file' => $file]);
    }
    ...
    {
        $entry = SurgeryFileEntry::where('filename', '=', $filename)->firstOrFail();
    ...
    protected function createFile($file, $userId, $patient_id, $description, $category)
    {
        ...
        $file = SurgeryFileEntry::create(array(
            'user_id'           => $userId,
    ...
    {
        $fileEntry = SurgeryFileEntry::where('id', $data['fid'])->where('user_id', $this->auth->id())->where('patient_id', $data['pid'])->first();
