...
use App\DRDSB\File\ReportFileEntry;
...
class ReportsFilesController extends Controller
{
    ...
    {
        return Datatables::of(ReportFileEntry::where('user_id', $this->auth->id())->where('patient_id', $request->get('pid')))
            ...
            ->addColumn('thumbnail', function ($fileEntry) {
                ...
                if (strpos($fileEntry->mime, 'image') !== false) {
                    ...
                    return '<a href="#" class="dt-view-report-file" data-remodal-target="file-viewer" data-fid="' . $fileEntry->id . '"><img class="img-thumbnail-table" src="' . $src . '"></a>';
                    ...
                    return '<a href="' . url('media/report/download/' . $fileEntry->filename) . '"><img class="img-thumbnail-table" src="' . $src . '"></a>';
                }
            ...
            ->addColumn('action', function ($fileEntry) {
                return
                    ...
                    <ul class="dropdown-menu pull-right">
                        <li><a data-fid="' . $fileEntry->id . '" data-remodal-target="update-report-file" class="dt-update-report-file" href="#">Editar información archivo</a></li>
                        <li><a data-fid="' . $fileEntry->id . '" data-remodal-target="remove-report-file" class="dt-remove-report-file" href="#">Eliminar archivo</a></li>
                    </ul>
                </div>';
            })
            ->make(true);
    ...
    {
        $fid  = $request->get('fid');
        $file = ReportFileEntry::find($fid);

        return view('doctor.media.patient.report.view', ['file' => $file]);
    }
    ...
    {
        $file = ReportFileEntry::find($id);

        return view('doctor.media.patient.report.ajax-update', ['file' => $file]);
    }
    ...
    {
        $entry = ReportFileEntry::where('filename', '=', $filename)->firstOrFail();
    ...
    protected function createFile($file, $userId, $patient_id, $description)
    {
        ...
        $file = ReportFileEntry::create(array(
            'user_id'           => $userId,
    ...
    {
        $fileEntry = ReportFileEntry::where('id', $data['fid'])->where('user_id', $this->auth->id())->where('patient_id', $data['pid'])->first();
