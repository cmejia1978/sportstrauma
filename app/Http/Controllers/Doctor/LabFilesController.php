...
use App\DRDSB\File\LabFileEntry;
...
class LabFilesController extends Controller
{
    ...
    {
        return Datatables::of(LabFileEntry::where('user_id', $this->auth->id())->where('patient_id', $request->get('pid')))
            ...
            ->addColumn('thumbnail', function ($fileEntry) {
                ...
                if (strpos($fileEntry->mime, 'image') !== false) {
                    ...
                    return '<a href="#" class="dt-view-lab-file" data-remodal-target="file-viewer" data-fid="' . $fileEntry->id . '"><img class="img-thumbnail-table" src="' . $src . '"></a>';
                    ...
                    return '<a href="' . url('media/lab/download/' . $fileEntry->filename) . '"><img class="img-thumbnail-table" src="' . $src . '"></a>';
                }
            ...
            ->addColumn('action', function ($fileEntry) {
                return
                    ...
                    <ul class="dropdown-menu pull-right">
                        <li><a data-fid="' . $fileEntry->id . '" data-remodal-target="update-lab-file" class="dt-update-lab-file" href="#">Editar información archivo</a></li>
                        <li><a data-fid="' . $fileEntry->id . '" data-remodal-target="remove-lab-file" class="dt-remove-lab-file" href="#">Eliminar archivo</a></li>
                    </ul>
                </div>';
            })
            ->make(true);
    ...
    {
        $fid  = $request->get('fid');
        $file = LabFileEntry::find($fid);

        return view('doctor.media.patient.lab.view', ['file' => $file]);
    }
    ...
    {
        $file = LabFileEntry::find($id);

        return view('doctor.media.patient.lab.ajax-update', ['file' => $file]);
    }
    ...
    {
        $entry = LabFileEntry::where('filename', '=', $filename)->firstOrFail();
    ...
    protected function createFile($file, $userId, $patient_id, $description)
    {
        ...
        $file = LabFileEntry::create(array(
            'user_id'           => $userId,
    ...
    {
        $fileEntry = LabFileEntry::where('id', $data['fid'])->where('user_id', $this->auth->id())->where('patient_id', $data['pid'])->first();
