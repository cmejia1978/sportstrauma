...
use App\DRDSB\File\AppointmentFileEntry;
...
class AppointmentFilesController extends Controller
{
    ...
    {
        return Datatables::of(AppointmentFileEntry::where('user_id', $this->auth->id())->where('appointment_id', $request->get('aid')))
            ...
            ->addColumn('thumbnail', function ($fileEntry) {
                ...
                if (strpos($fileEntry->mime, 'image') !== false) {
                    ...
                    return '<a href="#" class="dt-view-appointment-file" data-remodal-target="file-viewer" data-fid="' . $fileEntry->id . '"><img class="img-thumbnail-table" src="' . $src . '"></a>';
                    ...
                    return '<a href="' . url('media/appointment/download/' . $fileEntry->filename) . '"><img class="img-thumbnail-table" src="' . $src . '"></a>';
                }
            ...
            ->addColumn('action', function ($fileEntry) {
                return
                    ...
                    <ul class="dropdown-menu pull-right">
                        <li><a data-fid="' . $fileEntry->id . '" data-remodal-target="update-appointment-file" class="dt-update-appointment-file" href="#">Editar información archivo</a></li>
                        <li><a data-fid="' . $fileEntry->id . '" data-remodal-target="remove-appointment-file" class="dt-remove-appointment-file" href="#">Eliminar archivo</a></li>
                    </ul>
                </div>';
            })
            ->make(true);
    ...
    {
        $fid  = $request->get('fid');
        $file = AppointmentFileEntry::find($fid);

        return view('doctor.media.appointment.view', ['file' => $file]);
    }
    ...
    {
        $file = AppointmentFileEntry::find($id);

        return view('doctor.media.appointment.ajax-update', ['file' => $file]);
    }
        ...
        if ($file) {
            foreach ($file as $index => $new_file) {
                ...
                if ($validation->fails()) {
                    ...
                    $userId         = $this->auth->id();
                    $appointment_id = $formData['aid'][$index];
                    $desc           = $formData['description'][$index];
                    $upload         = $this->createFile($new_file, $userId, $appointment_id, $desc);
    ...
    {
        $entry = AppointmentFileEntry::where('filename', '=', $filename)->firstOrFail();
    ...
    protected function createFile($file, $userId, $appointment_id, $description)
    {
        ...
        $file = AppointmentFileEntry::create(array(
            'user_id'           => $userId,
    ...
    {
        $fileEntry = AppointmentFileEntry::where('id', $data['fid'])->where('user_id', $this->auth->id())->where('appointment_id', $data['aid'])->first();
